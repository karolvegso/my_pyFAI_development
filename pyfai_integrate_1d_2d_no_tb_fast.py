...
path_to_images = "d:/programs_work/Python/SimonMicky/Karol_learning_process/Simons_sterss_analysis_soft/images_from_synchrotron_for_processing/"
# specify root name of single image
image_root_name = "nca1_pristine"
...
path_to_calib = "d:/programs_work/Python/SimonMicky/Karol_learning_process/Simons_sterss_analysis_soft/some_calib_poni_file/params.poni"
# load pyFAI calibration poni file
ai = pyFAI.load(path_to_calib)
# insert path to pyFAI mask file
path_to_mask = "d:/programs_work/Python/SimonMicky/Karol_learning_process/Simons_sterss_analysis_soft/some_mask/some_mask.edf"
...
path_to_output_folder = "d:/programs_work/Python/SimonMicky/Karol_learning_process/Simons_sterss_analysis_soft/output_folder/"
...
radial_range_selector = [0.0 , 50.0]
...
azimuth_range_selector  = [-180 , 180]
...
radial_range_selector_2D = [0 , 50]
# select azimuthal range for 2D integration
azimuth_range_selector_2D = [-180, 180]
...
##    print(index_0)
    ...
    image_number_current = image_no_start + image_no_step * index_0
...
##    print(image_number_current_str)
    ...
    image_name = image_root_name + image_name_separator + image_number_current_str + image_name_extension
    print(image_name)
    # generate full path to current image
    path_to_image_full = path_to_images + image_name
##    print(path_to_image_full)
    # open single image using fabio module
    img = fabio.open(path_to_image_full)
...
##    plt.close()
    ...
    if (mask_switch == True):
        if (unit_of_radial_cut == "q_nm^-1"):
            q, intensity, sigma = ai.integrate1d_ng(img.data, npt = no_points_in_radial_cut, unit=unit_of_radial_cut, error_model=error_model_selector,
...
##            print(intensity)
            ...
            twotheta, intensity, sigma = ai.integrate1d_ng(img.data, npt = no_points_in_radial_cut, unit=unit_of_radial_cut,  error_model=error_model_selector,
                                                           radial_range = radial_range_selector, azimuth_range = azimuth_range_selector,
                                                           mask = mask_img.data,
                                                           polarization_factor = polarization_factor_selector, 
                                                           method = method_selector, 
...
##        q, intensity, sigma = ai.integrate1d_ng(img.data, no_points_in_cut, correctSolidAngle=True, unit=unit_of_azimcut, variance=variance_selector, error_model=error_model_selector,
##                                                radial_range = radial_range_selector, azimuth_range = azimuth_range_selector,
...
##                                                filename=path_to_azimuhal_cut)
##    else:
##        twotheta, intensity, sigma = ai.integrate1d_ng(img.data, no_points_in_cut, correctSolidAngle=True, unit=unit_of_azimcut, variance=variance_selector, error_model=error_model_selector,
##                                                       radial_range = radial_range_selector, azimuth_range = azimuth_range_selector,
##                                                       polarization_factor = polarization_factor_selector, normalization_factor = normalization_factor_selector,
##                                                       dark = dark_selector, flat = flat_selector, method = method_selector, 
##                                                       filename=path_to_azimuhal_cut)
    ...
    if (mask_switch == True):
        intensity_integ_2D, radial, azimuthal, sigma_integ_2D = ai.integrate2d_ng(img.data, npt_rad = npt_rad_2D, npt_azim = npt_azim_2D, filename = path_to_integ_2D_images,
...
##        plt.close()
        ...
        intensity_integ_2D, radial, azimuthal = ai.integrate2d_ng(img.data, npt_rad = npt_rad_2D, npt_azim = npt_azim_2D, filename = path_to_integ_2D_images,
                       error_model=error_model_selector,
                       polarization_factor = polarization_factor_selector,
                       method = method_selector_2D, unit = unit_selector_2D)
##        # display image
...
##    ai.integrate2d_ng(img.data, npt_rad = npt_rad_2D, npt_azim = npt_azim_2D, filename = path_to_integ_2D,
##                   error_model=error_model_selector,
